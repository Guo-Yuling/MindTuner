...
class _MeditationPlayScreenState extends State<MeditationPlayScreen> {
  bool _isPlaying = false;                    // 当前播放状态（播放/暂停）
  Duration _position = Duration.zero;         // 当前播放位置
  Duration _duration = Duration.zero;         // 音频总时长
  DateTime? _startTime;                       // 冥想开始时间
  bool _isLooping = false;                    // 循环播放状态
  double _volume = 1.0;                       // 音量（0.0-1.0）
  bool _shouldStopAudioOnPop = true;          // 控制返回时是否停止音频
  bool _isRegenerating = false;               // 重新生成状态
  bool _hasRecordedTime = false;              // 是否已经记录过时长
